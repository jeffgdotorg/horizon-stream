...
public class FlowKafkaForwarder implements MessageConsumer<Message, Message> {
    ...
    public FlowKafkaForwarder(@Qualifier("kafkaByteArrayProducerTemplate") KafkaTemplate<String, byte[]> kafkaProducer,
        TenantIDGrpcServerInterceptor tenantIDGrpcInterceptor,
        @Value("${flows.results.kafka-topic:" + DEFAULT_FLOW_RESULTS_TOPIC + "}") String kafkaTopic) {
        this.kafkaPublisher = new KafkaGrpcMessagePublisher("flow", kafkaProducer, tenantIDGrpcInterceptor, kafkaTopic);
    }
